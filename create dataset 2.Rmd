...
# estimating the nelson-alan estimator manually
nel_al_surg0 <- cumsum(sum_surg$n.event[sum_surg$strata=='surgery=0']/sum_surg$n.risk[sum_surg$strata=='surgery=0']) # calculating Nelson-Aalen estimator for surgery=0
nel_al_surg1 <- cumsum(sum_surg$n.event[sum_surg$strata=='surgery=1']/sum_surg$n.risk[sum_surg$strata=='surgery=1']) # calculating Nelson-Aalen estimator for surgery=1
plot(sum_surg$time[sum_surg$strata=='surgery=0'], nel_al_surg0, type = 's', xlab = 'Time in days', ylab = 'Cumulative Hazard') # ploting cumulative hazard for surgery
lines(sum_surg$time[sum_surg$strata=='surgery=1'], nel_al_surg1, type = 's', lty = 2)
legend('topleft', legend=c("Surgery = 0", "Surgery = 1"), lty=1:2, cex=.8)

# estimating the nelson-alan estimator with ggsurvplot
ggsurvplot(heart_surg, data = heart_data, conf.int = .90, fun = "cumhaz")


#### KM on surgery ####
heart_trans <- survfit(heart ~ transplant, data = heart_data, conf.type="plain", conf.int = .90) # fiting KM
heart_trans
sum_trans <- summary(heart_trans)
ggsurvplot(heart_trans, surv.median.line = "hv", conf.int = T, risk.table = T) # survival plot for transplant
